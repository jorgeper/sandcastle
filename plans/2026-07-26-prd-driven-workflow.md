...
### Task 6: Fork documentation — `README-FORK.md`, README callout, fork workflow conventions

**Fork-only task.** Everything here documents jorgeper's fork itself and must stay OUT of the feature commits: keep this task as its own final commit so Tasks 1–5 can be cherry-picked or PR'd upstream cleanly.

**Files:**

- Create: `README-FORK.md` (repo root)
- Modify: `README.md` (top — add a callout line immediately after the main title)
- Modify: `CLAUDE.md` (append a `## Fork workflow` section; `AGENTS.md` is a symlink to it, so it is picked up by both names)

- [ ] **Step 1: Create `README-FORK.md`**
...
- [ ] **Step 2: Add the callout to `README.md`**

Immediately after the main `# ...` title line at the top of `README.md`, insert:
...
- [ ] **Step 3: Append the fork workflow to `CLAUDE.md`**
...
- [ ] **Step 4: Commit (fork-docs only, single commit)**

```bash
git add README-FORK.md README.md CLAUDE.md
git commit -m "docs(fork): README-FORK change log, README callout, fork workflow conventions"
```
