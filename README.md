...
Releases are issues too: the scaffolded `/new-release` skill interviews you for a version (and optional targets), drafts the changelog for your approval, and files a `sandcastle:release` issue; `/cut-release <n>` then runs the cut in your Claude Code session — preflight from `.sandcastle/release-lane.mts` (strict semver, an ordering guard that refuses any version at or behind the newest tag, an abandoned-draft report), a `release/v<version>` branch, the gate, pre-tag CI, tag, merge-back, and draft verification, each phase logged as a marker comment on the issue so a re-run resumes. It stops at a verified draft; publishing is yours. Repo-specific mechanics go in the skill's one `release-facts` block (seeded with `TODO(sandcastle)` sentinels that halt the cut until filled). `release/*` branches are never deleted on merge. See `prd/009-release-lane.md` and ADR 0024.
